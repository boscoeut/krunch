...
async function placePeggedPerpOrder(market:string, client:MangoClient, 
    ...
    if (size > 3) throw new Error("Size must be less than 3")
...
(async () => {
    try {
        ...
        const user = accountDefinitions.find(account=>account.name === "FIVE")
        const client = await setupClient(user!, priorityFee);

        let transactionInstructions:Array<any> = [];
        // await deposit(2000, USDC_MINT, client.client, client.mangoAccount!, client.group);
         transactionInstructions.push(...(await borrow(2000, USDC_MINT, client.client, client.mangoAccount!, client.group)));
        // transactionInstructions.push(await cancelOpenOrders("SOL-PERP", client.client, client.mangoAccount!, client.group));
        // transactionInstructions.push(await placePerpOrder("SOL-PERP", client.client, client.mangoAccount!, client.group, PerpOrderSide.bid,1,173.50,1));
